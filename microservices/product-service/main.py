...
DB_PATH = Path(__file__).parent / "database" / "products.db"
...
app = FastAPI(title="VYRA Microservice — Product Service", version="1.0.0")
...
@app.get("/api/health")
def health():
    return {"service": "Product Service", "status": "healthy", "database": "products.db", "port": 8012}
...
def serve_gui():
    if gui_path.exists():
        with open(gui_path, "r", encoding="utf-8") as f:
            return f.read()
    return "<h1>Product Service GUI Missing</h1>"
