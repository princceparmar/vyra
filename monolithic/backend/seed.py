...
def seed_db():
    ...
    # Seed Default User
    demo_password_hash = hash_password("vyra2026")
    cursor.execute("""
    INSERT INTO users (name, email, password_hash)
    VALUES (?, ?, ?)
    """, ("Elena Rostova", "elena@vyra.fashion", demo_password_hash))

    cursor.execute("""
    INSERT INTO users (name, email, password_hash)
    VALUES (?, ?, ?)
    """, ("Marcus Vance", "marcus@vyra.fashion", demo_password_hash))
    ...
    conn.commit()
    conn.close()
    print(f"Seeded database with {len(PRODUCTS)} products and 2 demo users.")
